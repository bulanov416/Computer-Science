...
let plugString = plugTransform plugboard;;

let rec plugCipher (plugboard: string)(c: char) : char = 
	if (String.index plugString c) mod 2 = 0 then
		String.get plugboard (String.index plugboard c + 1)
	else
		String.get plugboard (String.index plugboard c -1)
;;

(*plugCipher plugString 'K';;*)
...
let cypher_character (c: char)(refl: string)(rotors: string list)(verbose: bool) : char =
	if verbose then begin
		Printf.printf "Enciphering %c\n" c;
		List.iteri (Printf.printf "rotor%d: [%s]\n") rotors;
		Printf.printf "reflB:  [%s]\n\n" refl;
	end;
	let cPlug1 = plugCipher plugString c in
	let c1 = transform cPlug1 (rotors @ [refl]) verbose in
	let c2 = rev_transform c1 (List.rev rotors) verbose in
	plugCipher plugString c2;;			
...
let rec main (refl: string)(rotors: string list)(notches: char list)(starts: char list)(s: string)(verbose: bool) : string =
	...
	let _c = cypher_character c refl _rotors verbose in (*Cipher the character*)
...
cipher reflB rotors notches starts runString;; 
